...
class iCal(object):
    ...
    def setClassTime(self):  # classTimeList
        data = []
        filepath = os.path.split(BASE_DIR)[0]
        with open(f'{filepath}/conf_classTime.json', 'r') as f:
            data = json.load(f)
    ...
    def icsCreateAndSave(self, username) -> uuid.UUID:
        ...
        return self.save(icsString, username)
...
class iCalPro(iCal):
    ...
    def iCalPro(self, username, password, date, reminder) -> tuple:
        ...
        try:
            ...
            filename = self.icsCreateAndSave(username)
